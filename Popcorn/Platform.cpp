...
const double AsPlatform::Max_Expanding_Size = 40.0; 
const double AsPlatform::Min_Expanding_Size = (double)Normal_Width;
const double AsPlatform::Expanding_Platform_Step = 1.0;
//------------------------------------------------------------------------------------------------------------
AsPlatform::AsPlatform()
: X_Pos(AsConfig::Border_X_Offset), Inner_Width(Normal_Platform_Inner_Width), Rolling_Step(0), Normal_Platform_Image_Width(0), 
  Normal_Platform_Image_Height(0), Last_Timer_Tick(0),Left_Key_Dowm(false), Right_Key_Down(false), Speed(0.0),Platform_Glue_Ratio(0.0), Expanding_Platform_Width(0.0),Normal_Platform_Image(0),Platform_Rect{}, 
  Prev_Platform_Rect{}, Highlight_Color(255, 255, 255), Platform_Circle_Color(151, 0, 0), Platform_Inner_Color(0, 128, 192), Truss_Color(Platform_Inner_Color,AsConfig::Global_Scale),
  Gun_Color(Highlight_Color, AsConfig::Global_Scale), Laser_Transformation_Step(0)
{
...
void AsPlatform::Set_State(EPlatform_State new_state)
{
	...
	case EPlatform_State::Expanding:
		...
		else
		{
			...
			Expanding_Platform_Width = AsPlatform::Min_Expanding_Size;
		}
		break;
...
void AsPlatform::Set_State(EPlatform_Substate_Regular new_state)
{
	...
	if(Platform_State == EPlatform_State::Regular && Platform_State.Regular == new_state)
		return;
		...
		switch(Platform_State)
		{
			case EPlatform_State::Glue:
			{
				...
				{//Финализация закончилась		
					Set_New_Or_Regular_State(new_state);
					return;
				}
				else
				{
...
void AsPlatform::Act_For_Expanding_State()
{
	...
	{
	case EPlatform_Transformation::Init:
		if(Expanding_Platform_Width < AsPlatform::Max_Expanding_Size)
		{
	...
	case EPlatform_Transformation::Finalize:
		if(Expanding_Platform_Width > AsPlatform::Min_Expanding_Size)
		{
			Expanding_Platform_Width -= Expanding_Platform_Step;
			X_Pos += Expanding_Platform_Step / 2.0;
		}
...
void AsPlatform::Draw_Expanding_Ball(HDC hdc, bool is_left)
{
	RECT rect, arc_rect;
	const double d_scale = AsConfig::D_Global_Scale;
	const int scale = AsConfig::Global_Scale;
...
//------------------------------------------------------------------------------------------------------------
void AsPlatform::Draw_Inner_Part(HDC hdc)
{
...
//------------------------------------------------------------------------------------------------------------
double AsPlatform::Get_Expanding_Value(HDC hdc, double start_pos, double end_pos, double ratio)
{
	double delta;
	double value;
...
//------------------------------------------------------------------------------------------------------------
void AsPlatform::Set_New_Or_Regular_State(EPlatform_Substate_Regular new_state)
{
	EPlatform_State next_state;

	Platform_State =  EPlatform_State::Regular;

	// Если есть отложенное состояние, то переводим в него
